...
/// 尽可能保住其余内容。
fn write_zip(out_path: &Path, files_root: &Path, entries: &[(String, String)]) -> Result<(), String> {
...
/// 逐项导出循环：构建与写盘都在阻塞线程上进行，每项是
/// （输出文件名, 实际写文件的闭包）。单项失败记入 failures，不中断整体；
/// 取消标志置位后在下一个条目前停止。
async fn export_loop<B, F>(
    app: tauri::AppHandle,
